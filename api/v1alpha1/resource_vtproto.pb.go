...
func (this *Metadata) EqualVT(that *Metadata) bool {
	...
	if equal, ok := interface{}(this.Updated).(interface {
		EqualVT(*timestamppb.Timestamp) bool
	}); ok {
		if !equal.EqualVT(that.Updated) {
			return false
		}
	} else if !proto.Equal(this.Updated, that.Updated) {
		return false
	}
...
func (m *Spec) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.YamlSpec) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.YamlSpec)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.ProtoSpec) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.ProtoSpec)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
...
func (m *IDQuery) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.Regexp) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.Regexp)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
